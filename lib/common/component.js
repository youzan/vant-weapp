...
function makeBaiduRelation(options, vantOptions, relation) {
    var _a;
    var created = vantOptions.created, beforeCreate = vantOptions.beforeCreate, destroyed = vantOptions.destroyed;
    var type = relation.type, name = relation.name, current = relation.current, linked = relation.linked, unlinked = relation.unlinked;
    if (type === 'ancestor') {
...
function VantComponent(vantOptions) {
    ...
    if (relation) {
        platform_1.isWechat && makeRelation(options, vantOptions, relation);
        platform_1.isBaidu && makeBaiduRelation(options, vantOptions, relation);
    }
