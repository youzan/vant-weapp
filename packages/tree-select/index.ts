...
VantComponent({
  ...
  methods: {
    ...
    onSelectItem(event: Weapp.Event) {
      ...
      // 判断有没有超出右侧选择的最大数
      const isOverMax = Array.isArray(this.data.activeId) && (this.data.activeId.length >= this.data.max);
      // 判断该项有没有被选中, 如果有被选中，则忽视是否超出的条件
      const isSelected = Array.isArray(this.data.activeId) ? this.data.activeId.indexOf(item.id) > -1 : this.data.activeId === item.id;
